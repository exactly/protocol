...
run(TASK_DEPLOY_RUN_DEPLOY, { reset: true, tags: "Markets" })
  .then(async () => {
    const [signers, marketDAI, marketUSDC, marketWBTC, auditor, dai, usdc, wbtc, priceFeedWBTC] = await Promise.all([
      getSigners(),
      getContract<Market>("MarketDAI"),
      getContract<Market>("MarketUSDC"),
      getContract<Market>("MarketWBTC"),
      getContract<Auditor>("Auditor"),
      getContract<MockERC20>("DAI"),
      getContract<MockERC20>("USDC"),
    ...
    for (const signer of signers) {
      ...
      await tx(usdc.connect(signer).approve(marketUSDC.address, MaxUint256, overrides));
      await tx(wbtc.connect(signer).approve(marketWBTC.address, MaxUint256, overrides));
      await tx(dai.connect(signer).mint(signer.address, MaxUint256.div(signers.length), overrides));
      await tx(usdc.connect(signer).mint(signer.address, MaxUint256.div(signers.length), overrides));
      await tx(wbtc.connect(signer).mint(signer.address, MaxUint256.div(signers.length), overrides));
      await tx(auditor.connect(signer).enterMarket(marketDAI.address, overrides));
      await tx(auditor.connect(signer).enterMarket(marketUSDC.address, overrides));
      await tx(auditor.connect(signer).enterMarket(marketWBTC.address, overrides));
    }

    const [decimalsDAI, decimalsUSDC, decimalsWBTC, maxFuturePools, { timestamp }] = await Promise.all([
      dai.decimals(),
      usdc.decimals(),
      wbtc.decimals(),
    ...
    await tx(marketUSDC.deposit(666_666_666n * 10n ** BigInt(decimalsUSDC), signers[0].address, overrides));
