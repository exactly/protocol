...
contract IntegrationPreviewer {
  ...
  function fixedRepayPosition(
    ...
    if (pool.unassignedEarnings == 0) return assets;
    ...
    uint256 netUnassignedEarnings = pool.unassignedEarnings.mulWadDown(1e18 - market.backupFeeRate());
    if (netUnassignedEarnings == 0) return assets;
    uint256 r = netUnassignedEarnings.mulWadDown(k).divWadDown(backupSupplied);
