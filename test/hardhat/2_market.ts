...
describe("Market", function () {
  ...
  describe("small positions", () => {
    describe("WHEN depositing 3wei of a dai", () => {
      beforeEach(async () => {
        ...
        await marketDAI.depositAtMaturity(pools[0], 3, 0, maria.address);
        await provider.send("evm_increaseTime", [9_011]);
      });
      ...
      it("AND its not possible to borrow 3 wei of a dai", async () => {
        await expect(
          marketDAI.borrowAtMaturity(pools[0], 3, 6, maria.address, maria.address),
        ).to.be.revertedWithCustomError(auditor, "InsufficientAccountLiquidity");
      });

      describe("AND WHEN borrowing 1 wei of DAI", () => {
        beforeEach(async () => {
          await expect(await marketDAI.borrowAtMaturity(pools[0], 1, 1, maria.address, maria.address))
            .to.emit(marketDAI, "BorrowAtMaturity")
            .withArgs(pools[0], maria.address, maria.address, maria.address, 1, 0);
        });
        ...
        it("AND a 1 wei of DAI borrow is registered", async () => {
          expect((await marketDAI.fixedPools(pools[0]))[0]).to.equal(1);
        });
      });
    });
  });
  ...
  describe("WHEN depositing 100 DAI to a maturity pool", () => {
    beforeEach(async () => {
      pools = await futurePools(2);
      await expect(marketDAI.depositAtMaturity(pools[0], parseUnits("100"), parseUnits("100"), maria.address))
        .to.emit(marketDAI, "DepositAtMaturity")
        .withArgs(pools[0], maria.address, maria.address, parseUnits("100"), 0);
    });
    ...
    it("AND the Market registers a supply of 100 DAI for the account", async () => {
      expect((await marketDAI.fixedDepositPositions(pools[0], maria.address))[0]).to.equal(parseUnits("100"));
    });

    it("WHEN trying to borrow DAI THEN it reverts with INSUFFICIENT_LIQUIDITY since collateral was not deposited yet", async () => {
      await expect(
        marketDAI.borrowAtMaturity(pools[0], 1000, 2000, maria.address, maria.address),
      ).to.be.revertedWithCustomError(auditor, "InsufficientAccountLiquidity");
    });

    describe("AND WHEN depositing 50 DAI to the same maturity, as the same account", () => {
      beforeEach(async () => {
        await expect(marketDAI.depositAtMaturity(pools[0], parseUnits("50"), parseUnits("50"), maria.address))
          .to.emit(marketDAI, "DepositAtMaturity")
          .withArgs(pools[0], maria.address, maria.address, parseUnits("50"), 0);
      });
    ...
    describe("WHEN depositing collateral and borrowing 60 DAI from the same maturity", () => {
      ...
      beforeEach(async () => {
        ...
        await expect(tx)
          .to.emit(marketDAI, "BorrowAtMaturity")
          .withArgs(pools[0], maria.address, maria.address, maria.address, parseUnits("60"), 0);
        const { blockNumber } = (await tx.wait())!;
        timestamp = (await provider.getBlock(blockNumber))!.timestamp;
      });

      it("AND a 60 DAI borrow is registered", async () => {
        expect((await marketDAI.fixedPools(pools[0]))[0]).to.equal(parseUnits("60"));
      });
      ...
      describe("AND WHEN trying to repay 100 (too much)", () => {
        ...
        beforeEach(async () => {
          ...
          await marketDAI.repayAtMaturity(pools[0], parseUnits("100"), parseUnits("100"), maria.address);
        });
      ...
      describe("AND WHEN borrowing 60 DAI from another maturity AND repaying only first debt", () => {
        beforeEach(async () => {
          ...
          await marketDAI.borrowAtMaturity(pools[1], parseUnits("60"), parseUnits("60"), maria.address, maria.address);
          await marketDAI.repayAtMaturity(pools[0], parseUnits("60"), parseUnits("60"), maria.address);
        });

        it("THEN contract's state variable fixedBorrows registers the second maturity where the account borrowed from", async () => {
          const { fixedBorrows } = await marketDAI.accounts(maria.address);
          expect(decodeMaturities(fixedBorrows)).contains(pools[1]);
        });
      });

      describe("AND WHEN fully repaying the debt", () => {
        beforeEach(async () => {
          await expect(await marketDAI.repayAtMaturity(pools[0], parseUnits("60"), parseUnits("60"), maria.address))
            .to.emit(marketDAI, "RepayAtMaturity")
            .withArgs(pools[0], maria.address, maria.address, parseUnits("60"), parseUnits("60"));
        });
        ...
        describe("AND WHEN withdrawing collateral and maturity pool deposit", () => {
          beforeEach(async () => {
            ...
            await provider.send("evm_setNextBlockTimestamp", [pools[0] + 1]);
      ...
      describe("GIVEN the maturity pool matures", () => {
        ...
        it("WHEN trying to withdraw an amount of zero THEN it reverts", async () => {
          await expect(
            marketDAI.withdrawAtMaturity(pools[0], 0, 0, maria.address, maria.address),
      ...
      describe("AND WHEN partially (40DAI, 66%) repaying the debt", () => {
        beforeEach(async () => {
          await expect(marketDAI.repayAtMaturity(pools[0], parseUnits("40"), parseUnits("40"), maria.address))
            .to.emit(marketDAI, "RepayAtMaturity")
            .withArgs(pools[0], maria.address, maria.address, parseUnits("40"), parseUnits("40"));
        });
        ...
        describe("AND WHEN moving in time to 1 day after maturity", () => {
          ...
          beforeEach(async () => {
            await provider.send("evm_setNextBlockTimestamp", [pools[0] + 86_400]);
          ...
          describe("AND WHEN repaying the rest of the 20.4 owed DAI", () => {
            beforeEach(async () => {
              ...
              await marketDAI.repayAtMaturity(pools[0], amount, amount, maria.address);
            });
          ...
          describe("AND WHEN repaying more than what is owed (30 DAI)", () => {
            beforeEach(async () => {
              await marketDAI.repayAtMaturity(pools[0], parseUnits("30"), parseUnits("30"), maria.address);
            });
    ...
    describe("AND WHEN moving in time to maturity AND withdrawing from the maturity pool", () => {
      beforeEach(async () => {
        await provider.send("evm_setNextBlockTimestamp", [pools[0] + 1]);
        await expect(
          marketDAI.withdrawAtMaturity(pools[0], parseUnits("100"), parseUnits("100"), maria.address, maria.address),
        )
          .to.emit(marketDAI, "WithdrawAtMaturity")
          .withArgs(pools[0], maria.address, maria.address, maria.address, parseUnits("100"), parseUnits("100"));
      });
  ...
  describe("maturity clearing:", () => {
    describe("GIVEN maria borrows from the first, third and fifth maturity", () => {
      beforeEach(async () => {
        ...
        pools = await futurePools(5);

        await marketDAI
          .connect(maria)
          .borrowAtMaturity(pools[0], parseUnits("1"), parseUnits("1"), maria.address, maria.address);
        await marketDAI
          .connect(maria)
          .borrowAtMaturity(pools[2], parseUnits("1"), parseUnits("1"), maria.address, maria.address);
        await marketDAI
          .connect(maria)
          .borrowAtMaturity(pools[4], parseUnits("1"), parseUnits("1"), maria.address, maria.address);
      });

      describe("WHEN she repays the first maturity borrow", () => {
        beforeEach(async () => {
          pools = await futurePools(5);
          await marketDAI.connect(maria).repayAtMaturity(pools[0], parseUnits("1"), parseUnits("1"), maria.address);
        });
        ...
        describe("AND WHEN she repays the third maturity borrow", () => {
          beforeEach(async () => {
            await marketDAI.connect(maria).repayAtMaturity(pools[2], parseUnits("1"), parseUnits("1"), maria.address);
          });
          ...
          describe("AND WHEN she repays the fifth maturity borrow", () => {
            beforeEach(async () => {
              await marketDAI.connect(maria).repayAtMaturity(pools[4], parseUnits("1"), parseUnits("1"), maria.address);
            });
  ...
  describe("GIVEN an interest rate of 2%", () => {
    beforeEach(async () => {
      ...
      pools = await futurePools(1);
      // add liquidity to the maturity
      await marketDAI.depositAtMaturity(pools[0], parseUnits("1"), parseUnits("1"), maria.address);
    });

    it("WHEN trying to borrow 0.8 DAI with a max amount of debt of 0.8 DAI, THEN it reverts with TOO_MUCH_SLIPPAGE", async () => {
      await expect(
        marketDAI.borrowAtMaturity(pools[0], parseUnits("0.8"), parseUnits("0.8"), maria.address, maria.address),
    ...
    it("WHEN trying to deposit 100 DAI with a minimum required amount to be received of 103, THEN 102 are received instead AND the transaction reverts with TOO_MUCH_SLIPPAGE", async () => {
      await expect(
        marketDAI.depositAtMaturity(pools[0], parseUnits("100"), parseUnits("103"), maria.address),
      ).to.be.revertedWithCustomError(marketDAI, "Disagreement");
    });
  });
  ...
  describe("GIVEN John deposited 12 DAI to the smart pool AND Maria borrowed 6 DAI from an empty maturity", () => {
    beforeEach(async () => {
      ...
      await marketDAI.borrowAtMaturity(pools[1], parseUnits("6"), parseUnits("6"), maria.address, maria.address);
    });

    it("WHEN Maria tries to borrow 5.99 more DAI on the same maturity, THEN it does not revert", async () => {
      await expect(
        marketDAI.borrowAtMaturity(pools[1], parseUnits("5.99"), parseUnits("5.99"), maria.address, maria.address),
      ).to.not.be.reverted;
    });
    ...
    it("WHEN Maria tries to borrow 6 more DAI on the same maturity (remaining liquidity), THEN it does not revert", async () => {
      await expect(marketDAI.borrowAtMaturity(pools[1], parseUnits("6"), parseUnits("6"), maria.address, maria.address))
        .to.not.be.reverted;
    });
    ...
    it("WHEN Maria tries to borrow 6.01 more DAI on another maturity, THEN it fails with InsufficientProtocolLiquidity", async () => {
      await expect(
        marketDAI.borrowAtMaturity(pools[1], parseUnits("6.01"), parseUnits("7"), maria.address, maria.address),
    ...
    it("WHEN Maria tries to borrow 12 more DAI on the same maturity, THEN it fails with InsufficientProtocolLiquidity", async () => {
      await expect(
        marketDAI.borrowAtMaturity(pools[1], parseUnits("12"), parseUnits("12"), maria.address, maria.address),
      ).to.be.revertedWithCustomError(marketDAI, "InsufficientProtocolLiquidity");
    });
    ...
    describe("AND John deposited 2388 DAI to the smart pool", () => {
      ...
      it("WHEN Maria tries to borrow 2500 DAI from a maturity, THEN it fails with InsufficientProtocolLiquidity", async () => {
        await expect(
          marketDAI.borrowAtMaturity(pools[1], parseUnits("2500"), parseUnits("5000"), maria.address, maria.address),
        ).to.be.revertedWithCustomError(marketDAI, "InsufficientProtocolLiquidity");
      });
      ...
      it("WHEN Maria tries to borrow 150 DAI from a maturity, THEN it succeeds", async () => {
        await expect(
          marketDAI.borrowAtMaturity(pools[0], parseUnits("150"), parseUnits("150"), maria.address, maria.address),
        ).to.not.be.reverted;
      });
    ...
    describe("AND John deposited 100 DAI to maturity", () => {
      beforeEach(async () => {
        ...
        await marketDAI.connect(john).depositAtMaturity(pools[0], parseUnits("100"), parseUnits("100"), john.address);
      });
      ...
      describe("AND John deposited 1200 DAI to the smart pool", () => {
        beforeEach(async () => {
          await marketDAI
            .connect(john)
            .depositAtMaturity(pools[0], parseUnits("1200"), parseUnits("1200"), john.address);
        });

        it("WHEN Maria tries to borrow 1350 DAI, THEN it fails with InsufficientProtocolLiquidity", async () => {
          await expect(
            marketDAI.borrowAtMaturity(pools[0], parseUnits("1350"), parseUnits("2000"), maria.address, maria.address),
          ).to.be.revertedWithCustomError(marketDAI, "InsufficientProtocolLiquidity");
        });

        it("WHEN Maria tries to borrow 200 DAI, THEN it succeeds", async () => {
          await expect(
            marketDAI.borrowAtMaturity(pools[0], parseUnits("200"), parseUnits("200"), maria.address, maria.address),
          ).to.not.be.reverted;
        });
  ...
  describe("GIVEN maria has plenty of WETH collateral", () => {
    ...
    describe("AND GIVEN she deposits 1000DAI into the next two maturity pools AND other 500 into the smart pool", () => {
      ...
      describe("WHEN borrowing 1200 in the current maturity", () => {
        ...
        it("THEN all of the maturity pools funds are in use", async () => {
          const [borrowed, supplied] = await marketDAI.fixedPools(pools[0]);
        ...
        describe("AND borrowing 1100 in a later maturity ", () => {
          ...
          it("THEN all of the maturity pools funds are in use", async () => {
            const [borrowed, supplied] = await marketDAI.fixedPools(pools[1]);
            expect(borrowed).to.gt(supplied);
          });

          it("THEN the later maturity owes 100 to the smart pool", async () => {
            const mp = await marketDAI.fixedPools(pools[1]);
          ...
          describe("AND WHEN repaying 50 DAI in the later maturity", () => {
            beforeEach(async () => {
              await marketDAI.repayAtMaturity(pools[1], parseUnits("50"), parseUnits("50"), maria.address);
            });

            it("THEN 1050 DAI are borrowed", async () => {
              expect((await marketDAI.fixedPools(pools[1])).borrowed).to.equal(parseUnits("1050"));
            });
          ...
          describe("AND WHEN john deposits 800 to the later maturity", () => {
            beforeEach(async () => {
              await marketDAI
                .connect(john)
                .depositAtMaturity(pools[1], parseUnits("800"), parseUnits("800"), john.address);
            });

            it("THEN 1100 DAI are still borrowed", async () => {
              expect((await marketDAI.fixedPools(pools[1])).borrowed).to.equal(parseUnits("1100"));
            });
        ...
        describe("AND WHEN john deposits 100 to the same maturity", () => {
          beforeEach(async () => {
            await marketDAI
              .connect(john)
              .depositAtMaturity(pools[0], parseUnits("100"), parseUnits("100"), john.address);
          });

          it("THEN 1200 DAI are still borrowed", async () => {
            expect((await marketDAI.fixedPools(pools[0])).borrowed).to.equal(parseUnits("1200"));
          });
          ...
          it("THEN the maturity pool still owes 100 to the smart pool", async () => {
            const mp = await marketDAI.fixedPools(pools[0]);
            expect(mp.borrowed - mp.supplied).to.equal(parseUnits("100"));
          });
        ...
        describe("AND WHEN john deposits 300 to the same maturity", () => {
          beforeEach(async () => {
            await marketDAI
              .connect(john)
              .depositAtMaturity(pools[0], parseUnits("300"), parseUnits("300"), john.address);
          });
        ...
        describe("AND WHEN repaying 100 DAI", () => {
          ...
          it("THEN 1100 DAI are still borrowed", async () => {
            expect((await marketDAI.fixedPools(pools[0])).borrowed).to.equal(parseUnits("1100"));
          });
        ...
        describe("AND WHEN repaying 300 DAI", () => {
          beforeEach(async () => {
            await marketDAI.repayAtMaturity(pools[0], parseUnits("300"), parseUnits("300"), maria.address);
          });

          it("THEN 900 DAI are still borrowed", async () => {
            expect((await marketDAI.fixedPools(pools[0])).borrowed).to.equal(parseUnits("900"));
          });
        ...
        describe("AND WHEN repaying in full (1200 DAI)", () => {
          beforeEach(async () => {
            await marketDAI.repayAtMaturity(pools[0], parseUnits("1200"), parseUnits("1200"), maria.address);
          });
    ...
    describe("AND GIVEN she borrows 5k DAI", () => {
      beforeEach(async () => {
        ...
        await marketDAI.depositAtMaturity(pools[0], parseUnits("5000"), parseUnits("5000"), maria.address);
        await marketDAI.borrowAtMaturity(
          pools[0],
          parseUnits("5000"),
          parseUnits("5000"),
      ...
      describe("AND WHEN moving in time to 20 days after maturity", () => {
        beforeEach(async () => {
          await provider.send("evm_setNextBlockTimestamp", [pools[0] + 86_400 * 20]);
        });
      ...
      describe("AND WHEN moving in time to 20 days after maturity but repaying really small amounts within some days", () => {
        beforeEach(async () => {
          for (const days of [5, 10, 15, 20]) {
            ...
            await provider.send("evm_setNextBlockTimestamp", [pools[0] + 86_400 * days]);
          }
        });
    ...
    describe("Operations in more than one pool", () => {
      describe("GIVEN a smart pool supply of 100 AND a borrow of 30 in a first maturity pool", () => {
        beforeEach(async () => {
          ...
          await marketDAI.borrowAtMaturity(pools[0], parseUnits("30"), parseUnits("30"), maria.address, maria.address);
        });

        it("WHEN a borrow of 70 is made to the second mp, THEN it should not revert", async () => {
          await expect(
            marketDAI.borrowAtMaturity(pools[1], parseUnits("70"), parseUnits("70"), maria.address, maria.address),
          ).to.not.be.reverted;
        });
        ...
        describe("AND GIVEN a deposit to the first mp of 30 AND a borrow of 70 in the second mp", () => {
          beforeEach(async () => {
            await marketDAI.depositAtMaturity(pools[0], parseUnits("30"), parseUnits("30"), maria.address);
            await marketDAI.borrowAtMaturity(
              pools[1],
              parseUnits("70"),
              parseUnits("70"),
          ...
          it("WHEN a borrow of 30 is made to the first mp, THEN it should not revert", async () => {
            await expect(
              marketDAI.borrowAtMaturity(pools[0], parseUnits("30"), parseUnits("30"), maria.address, maria.address),
            ).to.not.be.reverted;
          });

          it("WHEN a borrow of 30.01 is made to the first mp, THEN it should fail with error InsufficientProtocolLiquidity", async () => {
            await expect(
              marketDAI.borrowAtMaturity(pools[0], parseUnits("31"), parseUnits("31"), maria.address, maria.address),
            ).to.be.revertedWithCustomError(marketDAI, "InsufficientProtocolLiquidity");
          });
          ...
          describe("AND GIVEN a flexible borrow of 15", () => {
            ...
            it("WHEN a borrow of 15 is made to the first mp, THEN it should not revert", async () => {
              await expect(
                marketDAI.borrowAtMaturity(pools[0], parseUnits("15"), parseUnits("15"), maria.address, maria.address),
              ).to.not.be.reverted;
            });
            ...
            describe("AND GIVEN a deposit of 100 in the first mp", () => {
              beforeEach(async () => {
                await marketDAI.depositAtMaturity(pools[0], parseUnits("100"), parseUnits("100"), maria.address);
              });
          ...
          describe("AND GIVEN a borrow of 30 in the first mp", () => {
            ...
            describe("AND GIVEN a smart pool supply of 30 AND a flexible borrow of 15", () => {
              ...
              it("AND WHEN a deposit of 0.01 is added to the mp, THEN the withdraw of 15.01 is not reverted", async () => {
                await marketDAI.depositAtMaturity(pools[0], parseUnits("0.01"), parseUnits("0.01"), maria.address);
                await expect(
                  marketDAI.withdrawAtMaturity(
                    pools[0],
                    parseUnits("15.01"),
              ...
              it("AND WHEN a repay in the first maturity of 0.01 is done, THEN the withdraw of 15.01 is not reverted", async () => {
                await marketDAI.repayAtMaturity(pools[0], parseUnits("0.01"), parseUnits("0.01"), maria.address);
                await expect(
                  marketDAI.withdrawAtMaturity(
                    pools[0],
                    parseUnits("15.01"),
              ...
              it("AND WHEN a repay in the second maturity of 0.01 is done, THEN the withdraw of 15.01 is not reverted", async () => {
                await marketDAI.repayAtMaturity(pools[1], parseUnits("0.01"), parseUnits("0.01"), maria.address);
                await expect(
                  marketDAI.withdrawAtMaturity(
                    pools[0],
                    parseUnits("15.01"),
    ...
    describe("Smart Pool Reserve", () => {
      describe("GIVEN a sp total supply of 100, a 10% smart pool reserve and a borrow for 80", () => {
        beforeEach(async () => {
          ...
          await marketDAI.borrowAtMaturity(pools[0], parseUnits("80"), parseUnits("80"), maria.address, maria.address);
        });

        it("AND WHEN trying to borrow 10 more, THEN it should not revert", async () => {
          await expect(
            marketDAI.borrowAtMaturity(pools[0], parseUnits("10"), parseUnits("10"), maria.address, maria.address),
          ).to.not.be.reverted;
        });
        ...
        describe("AND GIVEN a deposit of 10 to the maturity pool", () => {
          beforeEach(async () => {
            await marketDAI.depositAtMaturity(pools[0], parseUnits("10"), parseUnits("10"), maria.address);
          });

          it("AND WHEN trying to borrow 20 more, THEN it should not revert", async () => {
            await expect(
              marketDAI.borrowAtMaturity(pools[0], parseUnits("20"), parseUnits("20"), maria.address, maria.address),
            ).to.not.be.reverted;
          });
          ...
          describe("AND GIVEN a borrow of 10 to the maturity pool AND a withdraw of 10", () => {
            ...
            describe("AND GIVEN a repay of 5", () => {
              beforeEach(async () => {
                await marketDAI.repayAtMaturity(pools[0], parseUnits("5"), parseUnits("5"), maria.address);
              });

              it("WHEN trying to borrow 5 more, THEN it should not revert", async () => {
                await expect(
                  marketDAI.borrowAtMaturity(pools[0], parseUnits("5"), parseUnits("5"), maria.address, maria.address),
                ).to.not.be.reverted;
              });
